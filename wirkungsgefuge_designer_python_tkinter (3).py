...
class App(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("Wirkungsgefüge Mini")
        self.geometry("900x600")
        self.configure(bg="#111827")

        self.model = InfluenceModel()
        # Demo
        a = self.model.add_node("Preis", 240, 220)
        b = self.model.add_node("Nachfrage", 480, 220)
        c = self.model.add_node("Angebot", 360, 380)
    ...
    def on_left(self, e):
        ...
        if k:
            if e.state & 0x0001:  # Shift → start edge
                self.edge_from = k
                self.temp_line = self.canvas.create_line(e.x, e.y, e.x, e.y, fill="#aab2bd", dash=(4, 2), arrow=tk.LAST)
            else:
                self.drag_key = k
        else:
    ...
    def on_move(self, e):
        self.canvas.config(cursor="hand2" if self.hit_node(e.x, e.y) else "arrow")
    ...
    def redraw(self):
        ...
        for k, (x, y, label) in self.model.nodes.items():
            self.draw_node(x, y, label)

    def draw_node(self, x, y, label):
        r = self.R
        self.canvas.create_oval(x-r, y-r, x+r, y+r, fill="#1d2b53", outline="#c9d1d9", width=2)
        self.canvas.create_text(x, y, text=label, fill="#e6edf3", font=("Segoe UI", 10, "bold"), width=2*r-8)
    ...
    def draw_edge(self, x1, y1, x2, y2, w):
        ...
        color = "#2ea043" if float(w) >= 0 else "#f85149"
        self.canvas.create_line(sx, sy, ex, ey, arrow=tk.LAST, width=max(1.5, min(4.0, 1.5 + abs(float(w)))), fill=color, smooth=True)
        mx, my = (sx+ex)/2, (sy+ey)/2
        self.canvas.create_rectangle(mx-14, my-9, mx+14, my+9, fill="#0b1020", outline="")
        self.canvas.create_text(mx, my, text=f"{float(w):.2g}", fill="#e6edf3", font=("Segoe UI", 9, "bold"))
